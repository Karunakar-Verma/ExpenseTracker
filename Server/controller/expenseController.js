...
export const addExpense = async (req, res) => {
  try {
    ...
    if (!expenseType) {
      return res
        .status(400)
        .json({ message: "Please specify the expense type." });
    }
    ...
    return res.status(200).json({ message: "Expense added", savedExpense });
  } catch (error) {
...
export const getExpense = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Your report", result });
  } catch (error) {
